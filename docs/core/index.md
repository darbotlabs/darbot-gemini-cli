...
DG-CLI's core package (`packages/core`) is the backend portion of DG-CLI, handling communication with the Gemini API, managing tools, and processing requests sent from `packages/cli`. For a general overview of DG-CLI, see the [main documentation page](../index.md).
...
While the `packages/cli` portion of DG-CLI provides the user interface, `packages/core` is responsible for:
...
DG-CLI includes a model fallback mechanism to ensure that you can continue to use the CLI even if the default "pro" model is rate-limited.
