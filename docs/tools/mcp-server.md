# MCP servers with the DG-CLI

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with the DG-CLI.

## What is an MCP server?

An MCP server is an application that exposes tools and resources to the DG-CLI through the Model Context Protocol, allowing it to interact with external systems and data sources. MCP servers act as a bridge between the Gemini model and your local environment or other services like APIs.

An MCP server enables the DG-CLI to:
...
- **Access resources:** Read data from specific resources (though the DG-CLI primarily focuses on tool execution).

With an MCP server, you can extend the DG-CLI's capabilities to perform actions beyond its built-in features, such as interacting with databases, APIs, custom scripts, or specialized workflows.

## Core Integration Architecture

The DG-CLI integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
The DG-CLI supports three MCP transport types:
...
The DG-CLI uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.
...
When the DG-CLI starts, it performs MCP server discovery through the following detailed process:
...
This comprehensive integration makes MCP servers a powerful way to extend the DG-CLI's capabilities while maintaining security, reliability, and ease of use.
