...
This document describes the `web_fetch` tool for the DG-CLI.
...
## How to use `web_fetch` with the DG-CLI

To use `web_fetch` with the DG-CLI, provide a natural language prompt that contains URLs. The tool will ask for confirmation before fetching any URLs. Once confirmed, the tool will process URLs through Gemini API's `urlContext`.
