...
- **Q: How do I update DG-CLI to the latest version?**
  - A: If installed globally via npm, update DG-CLI using the command `npm install -g @darbotlabs/dg-cli@latest`. If run from source, pull the latest changes from the repository and rebuild using `npm run build`.

- **Q: Where are DG-CLI configuration files stored?**
...
- **Error: Command not found (when attempting to run DG-CLI).**
  - **Cause:** DG-CLI is not correctly installed or not in your system's PATH.
  - **Solution:**
    1.  Ensure DG-CLI installation was successful.
...
- **CLI is not interactive in "CI" environments**
  ...
  - **Solution:** If the `CI_` prefixed variable is not needed for the CLI to function, you can temporarily unset it for the command. e.g., `env -u CI_TOKEN dg-cli`
