...
DG-CLI offers several ways to configure its behavior, including environment variables, command-line arguments, and settings files. This document outlines the different configuration methods and available settings.
...
DG-CLI uses `settings.json` files for persistent configuration. There are three locations for these files:

- **User settings file:**
  - **Location:** `~/.gemini/settings.json` (where `~` is your home directory).
  - **Scope:** Applies to all DG-CLI sessions for the current user.
...
In addition to a project settings file, a project's `.gemini` directory can contain other project-specific files related to DG-CLI's operation, such as:
...
- **`bugCommand`** (object):
  ...
  - **Default:** `"urlTemplate": "https://github.com/darbotlabs/darbot-gemini-cli/issues/new?template=bug_report.yml&title={title}&info={info}"`
  - **Properties:**
...
- **`theme`** (string):
  - **Description:** Sets the visual [theme](./themes.md) for DG-CLI.
  - **Default:** `"Default"`
...
- **`telemetry`** (object)
  - **Description:** Configures logging and metrics collection for DG-CLI. For more information, see [Telemetry](../telemetry.md).
...
- **`--prompt <your_prompt>`** (**`-p <your_prompt>`**):
  - Used to pass a prompt directly to the command. This invokes DG-CLI in a non-interactive mode.
- **`--sandbox`** (**`-s`**):
...
By understanding and utilizing these configuration layers and the hierarchical nature of context files, you can effectively manage the AI's memory and tailor the DG-CLI's responses to your specific needs and projects.

## Sandboxing

The DG-CLI can execute potentially unsafe operations (like shell commands and file modifications) within a sandboxed environment to protect your system.
...
When `.gemini/sandbox.Dockerfile` exists, you can use `BUILD_SANDBOX` environment variable when running DG-CLI to automatically build the custom sandbox image:
...
To help us improve the DG-CLI, we collect anonymized usage statistics. This data helps us understand how the CLI is used, identify common issues, and prioritize new features.
