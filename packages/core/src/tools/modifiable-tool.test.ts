...
describe('modifyWithEditor', () => {
    ...
    it('should successfully modify content with VSCode editor', async () => {
      ...
      expect(fs.writeFileSync).toHaveBeenNthCalledWith(
        ...
        expect.stringContaining(
          path.join(tempDir, 'dg-cli-tool-modify-diffs'),
        ),
        currentContent,
        'utf8',
      );
  ...
  it('should create temp files with correct naming with extension', async () => {
    ...
    expect(oldFilePath).toMatch(/dg-cli-modify-test-file-old-\d+\.txt$/);
    expect(newFilePath).toMatch(/dg-cli-modify-test-file-new-\d+\.txt$/);
    expect(oldFilePath).toContain(`${tempDir}/dg-cli-tool-modify-diffs/`);
    expect(newFilePath).toContain(`${tempDir}/dg-cli-tool-modify-diffs/`);
  });
  ...
  it('should create temp files with correct naming without extension', async () => {
    ...
    expect(oldFilePath).toMatch(/dg-cli-modify-test-file-old-\d+$/);
    expect(newFilePath).toMatch(/dg-cli-modify-test-file-new-\d+$/);
    expect(oldFilePath).toContain(`${tempDir}/dg-cli-tool-modify-diffs/`);
    expect(newFilePath).toContain(`${tempDir}/dg-cli-tool-modify-diffs/`);
  });
});
