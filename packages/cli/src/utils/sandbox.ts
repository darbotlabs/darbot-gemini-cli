...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'dg-cli-sandbox';
const SANDBOX_NETWORK_NAME = 'dg-cli-sandbox';
const SANDBOX_PROXY_NAME = 'dg-cli-sandbox-proxy';
...
export async function start_sandbox(
  ...
  // determine full path for dg-cli to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under dg-cli repo
  //
  // note this can only be done with binary linked from dg-cli repo
  if (process.env.BUILD_SANDBOX) {
    if (!gcPath.includes('dg-cli/packages/')) {
      console.error(
        'ERROR: cannot build sandbox using installed gemini binary; ' +
          'run `npm link ./packages/cli` under dg-cli repo to switch to linked binary.',
      );
      process.exit(1);
    } else {
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? 'Try running `npm run build:all` or `npm run build:sandbox` under the dg-cli repo to build it locally, or check the image name and your network connection.'
        : 'Please check the image name, your network connection, or file an issue at https://github.com/darbotlabs/darbot-dg-cli if the issue persists.';
    console.error(
