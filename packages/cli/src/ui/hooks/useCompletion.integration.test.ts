...
import { Config, FileDiscoveryService } from '@darbotlabs/dg-cli-core';
...
vi.mock('@darbotlabs/dg-cli-core', async () => {
  const actual = await vi.importActual('@darbotlabs/dg-cli-core');
  return {
    ...actual,
...
describe('useCompletion git-aware filtering integration', () => {
  ...
  it('should not perform recursive search when disabled in config', async () => {
    ...
    expect(fs.readdir).toHaveBeenCalledWith(testCwd.replace(/\//g, require('path').sep), { withFileTypes: true });
  });
  ...
  it('should use glob for top-level @ completions when available', async () => {
    ...
    expect(result.current.suggestions).toEqual([
      { label: 'README.md', value: 'README.md' },
      { label: 'src/index.ts', value: require('path').join('src', 'index.ts') },
    ]);
  });
